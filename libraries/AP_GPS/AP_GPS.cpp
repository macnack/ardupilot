...
#if AP_GPS_BLENDED_ENABLED
    // The blended driver is allocated in AP_GPS::init(). A pre-arm check can
    // reach here before that runs: AP_Vehicle's scheduler delay callback pumps
    // MAVLink while long init steps block, so a GCS arm command can be
    // serviced part-way through a vehicle's init_ardupilot(). Dereferencing
    // the null driver killed the flight computer on the pad. Guard it, the
